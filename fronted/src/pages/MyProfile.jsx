...
const MyProfile = () => {
 ...
 const [isEdit,setIsEdit]=useState(false) //false name display  // we apply the edit button click then change all info //if its true then edit the info otherwise not 
    ...
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      {
        ...
        :   <img src={userData.image} className='w-16 rounded'/>
      }
     ...
     <p className='font-medium'>Address:</p>
     {
      ...
      <p>
        <input  className="bg-gray-50" type='text' onChange={(e)=> setUserData(prev => 
          ({...prev, address:{...prev.address, line1:e.target.value}}))} value={userData.address.line1}/>
        <hr/>
        <input className='bg-gray-50' type='text' onChange={(e)=> setUserData(prev => 
          ({...prev, address:{...prev.address, line2:e.target.value}}))} value={userData.address.line2}/>
      </p> :
